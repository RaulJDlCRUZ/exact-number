...
public class Solver {
    ...
    // This method checks if the current total is within 10 of the target number, which could be a great approximation
    ...
    public boolean rocketscience(int[] numToUse, int targetNum, boolean[] used, int[] temp, int tempIndex, int currentTotal, char[] operation, int[] totals, int opIndex) {
        ...
        // The base case: if the current total matches the target number or is a great approximation, call showSolution on "main" to print the solution and return true
        if (currentTotal == targetNum || approximation(currentTotal, targetNum)) {
            ...
            for (int i = 0; i < numToUse.length; i++) {
                ...
                // If the number has not been used yet, mark it as used and add it to the temporary array
                if (!used[i]) {
                    used[i] = true;
